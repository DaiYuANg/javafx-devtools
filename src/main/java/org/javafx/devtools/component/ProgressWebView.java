...
public class ProgressWebView extends StackPane {
  ...
  private final WebView wview = new WebView();
  ...
  private final ChangeListener<Number> progressListener = new ChangeListener<>() {
  ...
  public ProgressWebView() {
    ...
    wview.getEngine().getLoadWorker().stateProperty().addListener(stateChangeListener);
    wview.getEngine().getLoadWorker().progressProperty().addListener(progressListener);
  }

  public void doLoad(final String page) {
    val location = wview.getEngine().getLocation();
